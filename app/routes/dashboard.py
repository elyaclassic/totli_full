...
async def agent_dashboard(request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_auth)):
    ...
    agent_info = {
        ...
        'location': (
            f"{latest_location.latitude:.4f}, {latest_location.longitude:.4f}"
            if latest_location and latest_location.latitude is not None and latest_location.longitude is not None
            else agent.region or 'Noma\'lum'
        )
    }
...
async def delivery_dashboard(request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_auth)):
    ...
    for driver, delivery_count in drivers_query:
        ...
        if latest_location and latest_location.latitude is not None and latest_location.longitude is not None:
            location_text = f"{latest_location.latitude:.4f}, {latest_location.longitude:.4f}"
