...
async def csrf_middleware(request: Request, call_next):
    try:
        ...
        import traceback
        traceback.print_exc()
        return JSONResponse(status_code=403, content={"detail": "CSRF tekshiruv xatosi"})
...
async def _csrf_middleware_impl(request: Request, call_next):
    ...
    # Himoyalanmaydigan yo'llar (API login/static, token bilan himoyalangan PWA location)
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # create_stock_movement qoldiqni o'zi yangilaydi; oldindan stock.quantity ni o'zgartirmaymiz.
...
async def qoldiqlar_tovar_hujjat_revert(
    ...
    if doc.status != "confirmed":
        raise HTTPException(status_code=400, detail="Faqat tasdiqlangan hujjatning tasdiqini bekor qilish mumkin")
    used_adjustment_movement_ids = set()
    for item in doc.items:
        stock = db.query(Stock).filter(
            Stock.warehouse_id == item.warehouse_id,
            Stock.product_id == item.product_id,
        ).first()
        movement_query = db.query(StockMovement).filter(
        ...
        if used_adjustment_movement_ids:
            movement_query = movement_query.filter(~StockMovement.id.in_(used_adjustment_movement_ids))
        movement = movement_query.order_by(StockMovement.id.desc()).first()
        if stock and movement:
            used_adjustment_movement_ids.add(movement.id)
            expected_before = (item.quantity or 0) - (movement.quantity_change or 0)
            applied_change = (movement.quantity_after or 0) - expected_before
            if applied_change:
                create_stock_movement(
                    db=db,
                    warehouse_id=item.warehouse_id,
                    product_id=item.product_id,
                    quantity_change=-applied_change,
...
async def delete_production(
    ...
    if not production:
        raise HTTPException(status_code=404, detail="Buyurtma topilmadi")
    if production.status == "completed":
        raise HTTPException(status_code=400, detail="Yakunlangan buyurtmani avval bekor qiling")
