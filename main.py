...
async def _csrf_middleware_impl(request: Request, call_next):
    ...
    # Himoyalanmaydigan yo'llar (API login, static, signed PWA location)
    if (
        path in ("/login", "/api/agent/login", "/api/driver/login")
        or (path in ("/api/agent/location", "/api/driver/location") and method == "POST")
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # Adjustment hujjati qoldiqni absolut qiymatga qo'yadi; harakat faqat audit delta.
...
async def delete_production(
    ...
    if not production:
        raise HTTPException(status_code=404, detail="Buyurtma topilmadi")
    if production.status == "completed":
        raise HTTPException(status_code=400, detail="Yakunlangan buyurtmani avval tasdiqdan bekor qiling")
...
def _active_agent_from_token(db: Session, token: str) -> Optional[Agent]:
    user_data = get_user_from_token(token)
    if not user_data or user_data.get("user_type") != "agent":
        return None
    return db.query(Agent).filter(
        Agent.id == user_data.get("user_id"),
        Agent.is_active == True,
    ).first()


def _active_driver_from_token(db: Session, token: str) -> Optional[Driver]:
    user_data = get_user_from_token(token)
    if not user_data or user_data.get("user_type") != "driver":
        return None
    return db.query(Driver).filter(
        Driver.id == user_data.get("user_id"),
...
async def driver_location_update(
    ...
    try:
        driver = _active_driver_from_token(db, token)
        if not driver:
            return {"success": False, "error": "Invalid token"}
...
async def agent_orders(token: str, db: Session = Depends(get_db)):
    """Agent orders list"""
    try:
        agent = _active_agent_from_token(db, token)
        if not agent:
            return {"success": False, "error": "Invalid token"}
