...
async def _csrf_middleware_impl(request: Request, call_next):
    ...
    # Himoyalanmaydigan yo'llar (API login, static, token bilan ishlaydigan PWA location)
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # Qator miqdori absolyut qoldiq; StockMovement esa faqat farqni qo'llaydi.
    ...
    for movement in movements:
        if movement.quantity_change:
            create_stock_movement(
                ...
                document_number=doc.number,
                user_id=current_user.id if current_user else None,
                note=f"Qoldiq tuzatishni qaytarish: {doc.number}",
            )
    doc.status = "draft"
    db.commit()
    return RedirectResponse(url=f"/qoldiqlar/tovar/hujjat/{doc_id}", status_code=303)
...
def _can_confirm_warehouse_transfer(db: Session, current_user: User, transfer: WarehouseTransfer) -> bool:
    ...
    if any(w.responsible_id == current_user.id for w in warehouses):
        return True
    department_ids = {w.department_id for w in warehouses if w.department_id}
    if not department_ids:
        return False
    employee_filters = [Employee.user_id == current_user.id]
    if current_user.phone:
        employee_filters.append(Employee.phone == current_user.phone)
    employees = db.query(Employee).filter(
        Employee.is_active == True,
        or_(*employee_filters),
    ).all()
    return any(employee.department_id in department_ids for employee in employees)
...
async def warehouse_transfer_confirm(
    ...
    if not _can_confirm_warehouse_transfer(db, current_user, transfer):
        return RedirectResponse(url=f"/warehouse/transfers/{transfer_id}?error=" + quote("Ushbu ombor o'tkazmasini tasdiqlash uchun ruxsat yo'q."), status_code=303)
    ...
    claimed = db.query(WarehouseTransfer).filter(
        ...
        WarehouseTransfer.approved_at: approved_at,
...
async def complete_production(prod_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_auth)):
    ...
    if production.status == "cancelled":
        raise HTTPException(status_code=400, detail="Bekor qilingan buyurtmani yakunlab bo'lmaydi")
    recipe = db.query(Recipe).filter(Recipe.id == production.recipe_id).first()
    if not recipe:
        raise HTTPException(status_code=404, detail="Retsept topilmadi")
    err = _do_complete_production_stock(db, production, recipe)
...
async def delete_production(
    ...
    if not production:
        raise HTTPException(status_code=404, detail="Buyurtma topilmadi")
    if production.status == "completed":
        raise HTTPException(status_code=400, detail="Yakunlangan buyurtmani avval tasdiqdan qaytaring")
