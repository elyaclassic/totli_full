...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        # create_stock_movement is the single place that mutates Stock quantities.
        stock = db.query(Stock).filter(
...
async def qoldiqlar_tovar_hujjat_revert(
    ...
    for movement in movements:
        ...
        movement_totals[key] = movement_totals.get(key, 0) + (movement.quantity_change or 0)

    for (warehouse_id, product_id), quantity_change in movement_totals.items():
        if quantity_change == 0:
            continue
        create_stock_movement(
            db=db,
