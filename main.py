...
async def csrf_middleware(request: Request, call_next):
    try:
        ...
        import traceback
        traceback.print_exc()
        return JSONResponse(status_code=403, content={"detail": "CSRF token tekshiruvida xatolik"})
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # create_stock_movement applies the delta and writes the audit row.
...
async def qoldiqlar_tovar_hujjat_revert(
    ...
    movement_nets = (
        db.query(
            ...
            func.coalesce(func.sum(StockMovement.quantity_change), 0).label("net_change"),
        )
        .filter(
    ...
    for warehouse_id, product_id, net_change in movement_nets:
...
def _can_confirm_warehouse_transfer(db: Session, transfer: WarehouseTransfer, current_user: User) -> bool:
    """Admin, responsible warehouse user, or warehouse department employee can approve."""
    if not current_user:
        return False
    if current_user.role == "admin":
        return True
    warehouses = db.query(Warehouse).filter(
        Warehouse.id.in_([transfer.from_warehouse_id, transfer.to_warehouse_id])
    ).all()
    for warehouse in warehouses:
        if warehouse.responsible_id and warehouse.responsible_id == current_user.id:
            return True
    department_ids = [warehouse.department_id for warehouse in warehouses if warehouse.department_id]
...
async def warehouse_transfer_confirm(
    ...
    if not _can_confirm_warehouse_transfer(db, transfer, current_user):
        return RedirectResponse(url=f"/warehouse/transfers/{transfer_id}?error=" + quote("Bu o'tkazishni tasdiqlashga ruxsat yo'q."), status_code=303)
...
async def warehouse_transfer(
    ...
    from urllib.parse import quote
    return RedirectResponse(
        url="/warehouse/transfers/new?error=" + quote("Eski tezkor o'tkazish o'chirilgan. O'tkazish hujjati orqali saqlang va tasdiqlang."),
        status_code=303,
    )
    if not current_user:
        return RedirectResponse(url="/login", status_code=303)
...
def _do_complete_production_stock(db, production, recipe):
    ...
    # StockMovement writes the finished-goods stock increase and audit row.
    create_stock_movement(
        db=db,
