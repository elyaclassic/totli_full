...
async def _csrf_middleware_impl(request: Request, call_next):
    ...
    # Himoyalanmaydigan yo'llar (API login, static, token bilan ishlaydigan PWA API)
    if path in (
        "/login",
        "/api/agent/login",
        "/api/driver/login",
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # create_stock_movement qoldiqni ham yangilaydi; bu yerda faqat delta beriladi.
...
async def qoldiqlar_tovar_hujjat_revert(
    ...
    for movement in movements:
        movement_by_key.setdefault((movement.warehouse_id, movement.product_id), movement)
    ...
    for item in doc.items:
        ...
        movement = movement_by_key.get((item.warehouse_id, item.product_id))
        if not movement:
            if stock and abs((stock.quantity or 0) - (item.quantity or 0)) > 1e-9:
                raise HTTPException(status_code=400, detail="Qoldiqni tiklash uchun ombor harakati topilmadi")
            continue
        previous_quantity = (item.quantity or 0) - (movement.quantity_change or 0)
        if stock:
            stock.quantity = max(previous_quantity, 0)
            stock.updated_at = datetime.now()
        ...
        if not dest or (dest.quantity or 0) < item.quantity:
            ...
            return RedirectResponse(
                url="/warehouse/transfers?error=" + quote(
                    f"Qaytarish mumkin emas: qabul qiluvchi omborda «{name}» yetarli emas "
...
async def complete_production_stage(
    ...
    if production.status == "completed":
        return RedirectResponse(url="/production/orders", status_code=303)
    if production.status not in ("draft", "in_progress"):
        return RedirectResponse(url="/production/orders?error=invalid_status", status_code=303)
...
async def delete_production(
    ...
    if production.status == "completed":
        from urllib.parse import quote
        return RedirectResponse(
            url="/production/orders?error=delete&detail=" + quote("Yakunlangan buyurtmani o'chirishdan oldin tasdiqni bekor qiling."),
            status_code=303,
        )
