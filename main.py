...
async def csrf_middleware(request: Request, call_next):
    try:
        ...
        import traceback
        traceback.print_exc()
        return JSONResponse(status_code=403, content={"detail": "CSRF tekshiruvida xatolik"})
...
def create_stock_movement_record(
    ...
    """Create an audit movement when the caller already set the absolute stock."""
    movement = StockMovement(
...
def user_can_confirm_transfer(db: Session, user: User, transfer: WarehouseTransfer) -> bool:
    if not user:
        return False
    if user.role == "admin":
        return True
    ...
    department_ids = {
        ...
        if warehouse and warehouse.department_id
    }
...
async def warehouse_transfer_confirm(
    ...
    if not transfer:
        raise HTTPException(status_code=404, detail="Hujjat topilmadi")
    if not user_can_confirm_transfer(db, current_user, transfer):
        raise HTTPException(status_code=403, detail="Bu o'tkazmani tasdiqlashga ruxsat yo'q")
    ...
    ).update({"status": "confirming"}, synchronize_session=False)
    if claimed != 1:
        db.rollback()
        return RedirectResponse(url=f"/warehouse/transfers/{transfer_id}?error=" + quote("Hujjat allaqachon tasdiqlangan yoki jarayonda."), status_code=303)
    db.flush()
