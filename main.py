...
async def _csrf_middleware_impl(request: Request, call_next):
    ...
    # Token-authenticated mobile endpoints do not use browser CSRF tokens.
...
async def qoldiqlar_tovar_hujjat_tasdiqlash(
    ...
    for item in doc.items:
        ...
        # create_stock_movement o'zi Stock ni quantity_change ga yangilaydi.
...
async def qoldiqlar_tovar_hujjat_revert(
    ...
    for movement in movements:
        ...
        net_changes[key] = net_changes.get(key, 0) + (movement.quantity_change or 0)
    for (warehouse_id, product_id), net_change in net_changes.items():
        if net_change:
            create_stock_movement(
                db=db,
                warehouse_id=warehouse_id,
...
def _do_complete_production_stock(db, production, recipe):
    ...
    # create_stock_movement o'zi Stock ni yangilaydi.
    create_stock_movement(
        db=db,
