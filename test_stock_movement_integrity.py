...
def test_completed_production_must_be_reverted_before_delete():
    db = _session()
    admin = User(username="admin", password_hash="x", role="admin", is_active=True)
    production = Production(number="PR-DONE", quantity=1, status="completed")
    db.add_all([admin, production])
    db.commit()

    with pytest.raises(Exception) as exc_info:
        asyncio.run(delete_production(production.id, db, admin))

    assert getattr(exc_info.value, "status_code", None) == 400
    assert db.query(Production).filter_by(id=production.id).first() is not None
